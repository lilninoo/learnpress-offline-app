...
class COL_LMS_Packages extends COL_LMS_API_Base {
    ...
    private function estimate_package_size($course_id, $options) {
        ...
        $base_size = 1024 * 1024; // 1MB pour les métadonnées
        ...
        if ($curriculum) {
            foreach ($curriculum as $section) {
                ...
                if ($items) {
                    foreach ($items as $item) {
                        if ($item->get_item_type() === 'lp_lesson') {
                            $base_size += $this->estimate_lesson_size($item->get_id(), $options);
                        }
                    }
                }
            }
    ...
    private function estimate_lesson_size($lesson_id, $options) {
        $size = 10240; // 10KB pour le contenu texte
        
        if ($options['include_videos']) {
            $size += 50 * 1024 * 1024; // 50MB par vidéo estimé
        }
    ...
    private function process_lesson_for_package($lesson_id, $package_dir, $options) {
        ...
        if ($options['include_documents'] || $options['include_images']) {
            ...
            foreach ($attachments as $attachment) {
                ...
                if ($options['include_documents'] && 
                    (strpos($mime_type, 'application/') === 0 || strpos($mime_type, 'text/') === 0)) {
                    $include = true;
                }
    ...
    private function get_package_download_url($package_id) {
        // Cette URL pourrait pointer vers un endpoint sécurisé
