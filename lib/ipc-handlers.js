...
function setupIpcHandlers(ipcMain, context) {
  ...
  // Remplacer le handler 'check-auto-login' dans lib/ipc-handlers.js
...
ipcMain.handle('check-auto-login', async () => {
    try {
        ...
        console.log('[IPC] Check auto-login:', { hasToken: !!token, hasRefreshToken: !!refreshToken, apiUrl });
        
        if (!token || !apiUrl) {
            console.log('[IPC] Pas de token ou d\'URL API');
        ...
        try {
            ...
            console.log('[IPC] Vérification du token...');
            const result = await apiClient.verifySubscription();
            
            if (result.success && result.isActive) {
                console.log('[IPC] Token valide et abonnement actif');
                ...
                console.log('[IPC] Token valide mais abonnement inactif');
                // Token valide mais pas d'abonnement actif
                return { 
                    success: false, 
                ...
                console.log('[IPC] Vérification échouée:', result);
                throw new Error('Vérification échouée');
            }
        } catch (error) {
            console.log('[IPC] Erreur de vérification, tentative de refresh...');
            
            // Si le token est expiré mais qu'on a un refresh token
            if (refreshToken && error.response?.status === 401) {
                try {
                    // UNE SEULE tentative de refresh
                    console.log('[IPC] Tentative unique de refresh token...');
                    ...
                    if (refreshResult.success) {
                        ...
                        if (verifyResult.success && verifyResult.isActive) {
                            setApiClient(apiClient);
                            return { 
                                success: true, 
                                username: username,
                    ...
                    // Retourner false pour forcer une nouvelle connexion
                    return { 
                        success: false, 
                        error: 'token_expired',
            ...
            console.log('[IPC] Nettoyage des tokens suite à l\'erreur');
            store.delete('token');
            store.delete('refreshToken');
        }
...
// Dans lib/ipc-handlers.js, remplacez la fonction api-login :
...
ipcMain.handle('api-login', async (event, { apiUrl, username, password }) => {
    try {
        ...
        if (result.success) {
            ...
            // IMPORTANT : Sauvegarder le client API
            setApiClient(apiClient);
            ...
            // Envoyer un événement au renderer pour signaler le succès
