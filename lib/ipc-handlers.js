  ...
  ipcMain.handle('check-auto-login', async () => {
    try {
        ...
        try {
            ...
            if (result.success) {
                console.log('[IPC] Token valide, abonnement:', result.isActive);
                ...
                console.log('[IPC] Échec de vérification:', result);
                throw new Error('Vérification échouée');
            }
        } catch (error) {
            console.log('[IPC] Erreur de vérification:', error.message);
            
            // Si erreur 401 et qu'on a un refresh token, essayer UNE SEULE fois
            if (error.response?.status === 401 && refreshToken) {
                ...
                try {
                    ...
                    if (refreshResult.success) {
                        console.log('[IPC] Refresh réussi, nouveau token obtenu');
                        ...
                        // Revérifier avec le nouveau token
            ...
            // Si on arrive ici, nettoyer les tokens invalides
            console.log('[IPC] Nettoyage des tokens suite à l\'échec');
            store.delete('token');
    ...
    try {
        ...
        if (result.success) {
            ...
            if (mainWindow && !mainWindow.isDestroyed()) {
                console.log('[IPC] Envoi de l\'événement login-success');
                setTimeout(() => {
